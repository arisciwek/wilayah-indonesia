...
class SettingsController {
    ...
    public function render_general_section() {
        echo '<p>' . __('Konfigurasi pengaturan umum untuk plugin Wilayah Indonesia.', 'wilayah-indonesia') . '</p>';
    }

    public function render_datatable_field() {
        $options = get_option('wilayah_indonesia_settings');
        ...
        <p class="description">
            <?php _e('Jumlah data yang ditampilkan per halaman dalam tabel', 'wilayah-indonesia'); ?>
        </p>
        <?php
    }
    ...
    public function render_cache_field() {
        ...
        ?>
        <label>
            <input type="checkbox" name="wilayah_indonesia_settings[enable_cache]" 
        ...
        <div class="cache-options" style="margin-top: 10px;">
            <label>
                ...
                <input type="number" name="wilayah_indonesia_settings[cache_duration]" 
    ...
    public function render_debug_field() {
        ...
        ?>
        <label>
            <input type="checkbox" name="wilayah_indonesia_settings[enable_debug]" 
                   value="1" <?php checked($enable_debug, 1); ?>>
            <?php _e('Aktifkan mode debug', 'wilayah-indonesia'); ?>
        </label>
        <p class="description">
            <?php _e('Menampilkan informasi debugging tambahan di console', 'wilayah-indonesia'); ?>
        </p>
        <?php
    }
    ...
    public function renderPage() {
        ...
        require_once WILAYAH_INDONESIA_PATH . 'src/Views/templates/settings/settings_page.php';
        //require_once WILAYAH_INDONESIA_PATH . 'src/Views/templates/settings/tab-permissions.php';
        //require_once WILAYAH_INDONESIA_PATH . 'src/Views/templates/settings/tab-general.php';
...
private function loadTabView($tab) {
    ...
    $tab_file = WILAYAH_INDONESIA_PATH . 'src/Views/templates/settings/' . $allowed_tabs[$tab];
    
    if (file_exists($tab_file)) {
        require_once $tab_file;
    } else {
        echo sprintf(
            __('Tab file tidak ditemukan: %s', 'wilayah-indonesia'),
            esc_html($tab_file)
        );
    }
}
...
/*
    private function loadTabView($tab) {
        $tab_file = WILAYAH_INDONESIA_PATH . 'src/Views/templates/settings/tab-' . $tab . '.php';
